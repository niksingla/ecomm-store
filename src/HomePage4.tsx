...
const dummyProducts = [
  ...
  { id: 4, name: "EasyGlide Loafers", price: 59.99, category: "Casual", img: "https://images.unsplash.com/photo-1600585154340-be6161a56a0c" },
  ...
  { id: 7, name: "TrailTrek Boots", price: 149.99, category: "Boots", img: "https://images.unsplash.com/photo-1600585154340-be6161a56a0c" },
...
const HomePage4: React.FC = () => {
  ...
  const categories = [
    ...
    { id: 4, name: "Casual", img: "https://images.unsplash.com/photo-1600585154340-be6161a56a0c" },
  ];
  ...
  // Subsets of Products
  const featuredProducts = dummyProducts.slice(0, 5); // First 5 products
  const bestSellers = dummyProducts.slice(5, 10); // Last 5 products
    ...
    <div className="min-h-screen bg-[#FFFFFF] font-sans">
      ...
      <header className="bg-[#FFFFFF] shadow-md p-4 fixed top-8 left-0 w-full z-10 flex flex-col md:flex-row justify-between items-center">
        ...
        <div className="flex items-center w-full md:w-auto mb-2 md:mb-0">
          <select className="p-2 border-r-0 border-[#6366F1] rounded-l-md text-[#0A0908] focus:outline-none">
          ...
          <input
            ...
            className="p-2 w-full md:w-64 border border-[#6366F1] rounded-r-md text-[#0A0908] focus:outline-none"
          />
        </div>
        <nav className="hidden md:flex space-x-6">
          ...
          <a href="#" className="text-[#6366F1] hover:text-[#9333EA] font-medium">Sign Up</a>
        </nav>
        <div className="flex space-x-4">
          <button className="text-[#6366F1] hover:text-[#9333EA]">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
            </svg>
          </button>
          <button onClick={() => setIsCartOpen(true)} className="text-[#6366F1] hover:text-[#9333EA] relative">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      >
        <div className="bg-black bg-opacity-50 p-8 rounded-lg">
          <h2 className="text-4xl md:text-5xl font-bold text-[#FFFFFF] mb-4">Step Into Savings!</h2>
          <p className="text-lg md:text-xl text-[#FFFFFF] mb-6">Up to 50% Off on All Footwear</p>
          <a href="#" className="inline-block bg-[#6366F1] text-[#FFFFFF] px-8 py-3 rounded-full font-semibold hover:bg-[#9333EA]">
            Shop Now
          </a>
        </div>
      </section>
      ...
      <section className="p-6 bg-[#F5F5F5]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {categories.map((category) => (
            <div key={category.id} className="bg-[#FFFFFF] rounded-lg shadow-md p-4 text-center">
              <img src={category.img} alt={category.name} className="w-full h-32 object-cover rounded-md mb-2" />
              <h4 className="text-lg font-medium text-[#0A0908]">{category.name}</h4>
              <a href="#" className="text-[#6366F1] hover:text-[#9333EA]">Shop Now</a>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="p-6">
        <h3 className="text-2xl font-semibold text-[#0A0908] mb-4 text-center">Featured Footwear</h3>
      ...
      <section className="p-6 bg-[#F5F5F5]">
        <h3 className="text-2xl font-semibold text-[#0A0908] mb-4 text-center">Best Sellers</h3>
